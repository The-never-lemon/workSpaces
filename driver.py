...
def main():
    ...
    if device.type == 'cuda':
        torch.cuda.set_device(0)  # 显式初始化 CUDA 主上下文，避免 cuBLAS 报 CUDA context 缺失
    ...
    if n_gpus > 1:
        dp_policy = nn.DataParallel(global_policy_net, device_ids=list(range(n_gpus)))
        dp_q_net1 = nn.DataParallel(global_q_net1, device_ids=list(range(n_gpus)))
        dp_q_net2 = nn.DataParallel(global_q_net2, device_ids=list(range(n_gpus)))
        dp_target_q_net1 = nn.DataParallel(global_target_q_net1, device_ids=list(range(n_gpus)))
        dp_target_q_net2 = nn.DataParallel(global_target_q_net2, device_ids=list(range(n_gpus)))

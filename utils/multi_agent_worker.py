    ...
    def run_episode(self):
        ...
        for i in range(MAX_EPISODE_STEP):
            ...
            for robot in self.robot_list:
                ...
                node_scores, heading_logps, _ = robot.get_node_action_scores(observation)
                all_node_scores.append(node_scores)
                all_heading_logps.append(heading_logps)
                all_observations.append(observation)

            # Phase 2: Hungarian 联合分配 — 全局最优一对一节点匹配
            if USE_JOINT_ASSIGNMENT:
                try:
                    assignments = joint_assignment(all_node_scores)
                except Exception:
                    assignments = [max(scores, key=scores.get) if scores else 0 for scores in all_node_scores]
            else:
                assignments = [max(scores, key=scores.get) if scores else 0 for scores in all_node_scores]

            # Phase 3: 各 Agent 为分配到的节点选择朝向
            selected_locations = []
            dist_list = []
            next_node_index_list = []
            next_heading_index_list = []
            for robot, obs, assigned_node, h_logps in zip(
                self.robot_list, all_observations, assignments, all_heading_logps
            ):
                next_location, next_node_idx, heading_idx, action_idx = robot.select_heading_for_node(
                    obs, assigned_node, h_logps, greedy=False)

                robot.save_action(action_idx)
                selected_locations.append(next_location)
                dist_list.append(np.linalg.norm(next_location - robot.location))
                next_node_index_list.append(next_node_idx)
                next_heading_index_list.append(heading_idx)
            ...
            if USE_JOINT_ASSIGNMENT:
                for scores, assigned in zip(all_node_scores, assignments):
                    if scores:
                        original_best = max(scores, key=scores.get)
                        if original_best != assigned:
                            conflict_penalty += 0.05

            team_reward = self.env.calculate_team_reward() - 0.5 - conflict_penalty
            if done:
                team_reward += 10

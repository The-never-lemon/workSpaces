...
class Agent:
    ...
    def get_observation(self, pad=True):
        ...
        self.k_size = k_size  # 存储有效邻居数量，供 Hungarian 阶段过滤填充条目
        if pad:
    ...
    def get_node_action_scores(self, observation):
        """从策略网络输出中提取每个邻居节点的 log-sum-exp 分数，供 Hungarian 联合分配使用。"""
        _, _, _, _, current_edge, _, _, _, _ = observation
        with torch.no_grad():
            logp = self.policy_net(*observation)  # [1, K*3]

        logp = logp.squeeze(0)  # [K*3]
        # NaN 检测：模型权重数值不稳定时 logp 可能产生 NaN，后续传入 scipy 会导致 C 层 segfault
        if torch.isnan(logp).any():
            logp = torch.nan_to_num(logp, nan=-1e10, posinf=-1e10, neginf=-1e10)

        neighbor_indices = current_edge.squeeze()  # [K]

        node_scores = {}
        heading_logps = {}
        k_size = getattr(self, 'k_size', neighbor_indices.size(0))
        for k in range(k_size):
            node_idx = int(neighbor_indices[k].item())
            heading_logps_k = logp[k * self.num_heading_candidates:(k + 1) * self.num_heading_candidates]
            lse = torch.logsumexp(heading_logps_k, dim=0).item()
            if lse != lse or lse == float('-inf'):  # isnan or -inf
                lse = -1e10
            if node_idx not in node_scores:
                node_scores[node_idx] = lse
                heading_logps[node_idx] = heading_logps_k

        return node_scores, heading_logps, logp
    ...
    def select_heading_for_node(self, observation, assigned_node_idx, heading_logps, greedy=False):
        """为 Hungarian 分配到的节点选择最佳朝向，返回与 select_next_waypoint 一致的格式。"""
        _, _, _, _, current_edge, _, _, _, _ = observation

        if assigned_node_idx not in heading_logps:
            return self.select_next_waypoint(observation, greedy=greedy)

        h_logps = heading_logps[assigned_node_idx]  # [3]
        if greedy:
            heading_subidx = torch.argmax(h_logps).item()
        else:
            heading_subidx = torch.multinomial(h_logps.exp(), 1).item()

        # 找到 assigned_node_idx 在 current_edge 中的位置
        neighbor_indices = current_edge.squeeze()  # [K]
        matches = (neighbor_indices == assigned_node_idx).nonzero(as_tuple=True)
        if len(matches[0]) == 0:
            return self.select_next_waypoint(observation, greedy=greedy)
        waypoint_idx = matches[0][0].item()

        heading_index = int(self.neighbor_best_indices[waypoint_idx][heading_subidx]
                            if np.ndim(self.neighbor_best_indices[waypoint_idx]) == 1
                            else self.neighbor_best_indices[waypoint_idx][0][heading_subidx])
        action_index = torch.tensor([waypoint_idx * self.num_heading_candidates + heading_subidx])
        next_position = self.node_coords[assigned_node_idx]

        return next_position, assigned_node_idx, heading_index, action_index
